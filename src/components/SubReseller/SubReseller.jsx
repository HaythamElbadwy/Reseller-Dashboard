...
export default function SubReseller() {
  ...
  const addNewSubReseller = async () => {
    ...
    setIsLoading(true)
    try {
      const response = await fetch(`https://wishtv.onrender.com/reseller/addSubReseller`, {
        method: 'POST',
        headers: {
  ...
  const getSubReseller = async () => {
    ...
    try {
      const response = await fetch(`https://wishtv.onrender.com/reseller/getSubReseller`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          'authorization': `wisOZ0${localStorage.getItem('authToken')}`
